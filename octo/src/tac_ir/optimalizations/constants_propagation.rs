...
pub fn propagate_constants(code: PipelineIR) -> PipelineIR {
    ...
    while let Some(x) = operations.pop() {
        ...
        let result_operation = match propagate_constant_operation(
            &mut ctx,
            &mut operations,
            x,
            result_address,
...
pub fn propagate_constant_operation(
    ...
    let result_operation = match x {
        ...
        Store(addr) => match ctx.get_const(&addr) {
            None => Some(x),
            Some(val) => {
        ...
        ConstructVec2(addr1, addr2) => match (ctx.get_const(&addr1), ctx.get_const(&addr2)) {
            (Some(v1), Some(v2)) => {
                let f1 = match v1 {
        ...
        ExtractComponent(addr, id) => match ctx.get_const(&addr) {
            Some(val) => match val {
        ...
        StoreComponent(addr, id, addr2) => match (ctx.get_const(&addr), ctx.get_const(&addr2)) {
            (Some(v1), Some(v2)) => {
                let f = match v2 {
        ...
        Add(addr1, addr2) => match (ctx.get_const(&addr1), ctx.get_const(&addr2)) {
            (Some(v1), Some(v2)) => {
                use ConstantValue::*;
                let val = match (v1, v2) {
                    ...
                    (Vec3(a1), Vec3(a2)) => {
                        ConstantValue::Vec3([a1[0] + a2[0], a1[1] + a2[1], a1[2] + a2[2]])
                    }
                    (Vec4(a1), Vec4(a2)) => ConstantValue::Vec4([
                        a1[0] + a2[0],
                        a1[1] + a2[1],
                        a1[2] + a2[2],
                        a1[3] + a2[3],
        ...
        Sub(addr1, addr2) => match (ctx.get_const(&addr1), ctx.get_const(&addr2)) {
            (Some(v1), Some(v2)) => {
                use ConstantValue::*;
                let val = match (v1, v2) {
                    (Float(a1), Float(a2)) => ConstantValue::Float(a1 - a2),
        ...
        Mul(addr1, addr2) => match (ctx.get_const(&addr1), ctx.get_const(&addr2)) {
            (Some(v1), Some(v2)) => {
                use ConstantValue::*;
                let val = match (v1, v2) {
                    (Float(a1), Float(a2)) => ConstantValue::Float(a1 * a2),
        ...
        Div(addr1, addr2) => match (ctx.get_const(&addr1), ctx.get_const(&addr2)) {
            (Some(v1), Some(v2)) => {
                use ConstantValue::*;
                let val = match (v1, v2) {
                    (Float(a1), Float(a2)) => ConstantValue::Float(a1 / a2),
        ...
        Less(addr1, addr2) => match (ctx.get_const(&addr1), ctx.get_const(&addr2)) {
            (Some(v1), Some(v2)) => {
                use ConstantValue::*;
                let val = match (v1, v2) {
        ...
        LessEq(addr1, addr2) => match (ctx.get_const(&addr1), ctx.get_const(&addr2)) {
            (Some(v1), Some(v2)) => {
                use ConstantValue::*;
                let val = match (v1, v2) {
                    (Float(a1), Float(a2)) => ConstantValue::Bool(a1 <= a2),
        ...
        Eq(addr1, addr2) => match (ctx.get_const(&addr1), ctx.get_const(&addr2)) {
            (Some(v1), Some(v2)) => {
                use ConstantValue::*;
                let val = match (v1, v2) {
                    (Float(a1), Float(a2)) => ConstantValue::Bool(a1 == a2),
                    ...
                    (Vec3(a1), Vec3(a2)) => {
                        ConstantValue::Bool(a1[0] == a2[0] && a1[1] == a2[1] && a1[2] == a2[2])
                    }
                    (Vec4(a1), Vec4(a2)) => ConstantValue::Bool(
        ...
        Neq(addr1, addr2) => match (ctx.get_const(&addr1), ctx.get_const(&addr2)) {
            (Some(v1), Some(v2)) => {
                use ConstantValue::*;
                let val = match (v1, v2) {
                    (Float(a1), Float(a2)) => ConstantValue::Bool(a1 != a2),
        ...
        Neg(addr) => match ctx.get_const(&addr) {
            Some(v) => {
                let val = match v {
        ...
        And(addr1, addr2) => match (ctx.get_const(&addr1), ctx.get_const(&addr2)) {
            (Some(v1), Some(v2)) => {
                let val = match (v1, v2) {
                    (ConstantValue::Bool(x1), ConstantValue::Bool(x2)) => {
                        ConstantValue::Bool(x1 && x2)
                    }
                    _ => unreachable!(),
                };
                Some(ctx.copy_const(result_address, val))
        ...
        Or(addr1, addr2) => match (ctx.get_const(&addr1), ctx.get_const(&addr2)) {
            (Some(v1), Some(v2)) => {
                let val = match (v1, v2) {
                    (ConstantValue::Bool(x1), ConstantValue::Bool(x2)) => {
                        ConstantValue::Bool(x1 || x2)
        ...
        Sync(addr) => {
            ...
            match ctx.get_const(&addr) {
                Some(y) => Some(ctx.copy_const(result_address, y)),
                None => Some(x),
            }
        }
        Shift(..) => Some(x),
        ...
        Jump(lab) => Some(if label_map.contains_key(&lab) {
            Operation::Jump(label_map[&lab])
        } else {
            x
        }),
        JumpIfElse(cond, true_label, false_label) => {
            if propagate_jump_if_else(
                ctx,
                cond,
                true_label,
                false_label,
